...
PROGRAM(1, AUTOSTART)
{
    ...
    {
        if (i != os_getCurrentProc()) { os_kill(i); }
    }
    ...
    lcd_clear();
    lcd_writeProgString(PSTR("Phase 3: 100x   Crit. Sec."));
    delayMs(2000);

    os_exec(9, DEFAULT_PRIORITY);
    os_yield();

    // You should only be here if program 9 has terminated correctly.
    lcd_goto(2,14);
    ...
    lcd_clear();
    lcd_writeProgString(PSTR("Phase 4: Processkills itself"));
    delayMs(2000);

    os_exec(10, DEFAULT_PRIORITY);
    os_yield();
...
//! This program starts 100 critical sections and terminates without closing them.
PROGRAM(9, DONTSTART)
{
    for (uint8_t i = 0; i < 100; i++){ os_enterCriticalSection(); }
}

//! This program kills itself and throws an error when os_kill returns to killed process.
PROGRAM(10, DONTSTART)
{
    os_kill(os_getCurrentProc());
    os_error("os_kill return to the killed process");
}
