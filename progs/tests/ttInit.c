...
//          TestSuite: Init
//-------------------------------------------------
// Tests if the process stack is correctly
// initialized with zeros for all processor
// registers.
//-------------------------------------------------
#include "../progs.h"
#if defined(TESTTASK_ENABLED) && TESTTASK == TT_INIT
...
#define BASE_ADDRESS (0x21FF-TEST_PROGRAMS)
//! Address for register-buffer
#define BUFFER_ADDRESS (BASE_ADDRESS-1)

#warning This testtask can only be used in exercise 2 since it is not compatible with the dispatcher
...
PROGRAM(1, AUTOSTART) {
    ...
    for (;;) {
        ...
        for (uint8_t i = 0; i < TEST_PROGRAMS; i++) {
			...
			{
	            case 0xFF:
					lcd_writeProgString(PSTR("??")); //Testtask is not working
					success = 0;
					break;
	            case 0xAA:
        ...
        if (success) {
            delayMs(3000); // Give a chance to see the result
            lcd_clear();
            for (;;) {
