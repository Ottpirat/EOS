...
//-------------------------------------------------
#include "../progs.h"
#if defined(TESTTASK_ENABLED) && TESTTASK == TT_HEAP_CLEANUP
...
volatile process_id_t allocProc = 0;
...
PROGRAM(1, AUTOSTART)
{
    ...
    if (os_getHeapListLength() != 1 || os_lookupHeap(0) == os_lookupHeap(1))
	{
...
// 1. Phase: Test if you can over-allocate memory
    ...
    for (i = 0; i < DRIVERS; i++)
	{
        heap_t*     heap  = os_lookupHeap(i);
        uint16_t  total = os_getUseSize(heap);
        if (os_malloc(heap, total)) { hc_error(PSTR("Overalloc failure")); }
    }
...
// 2. Phase: Settings test: Check if heap is set up correctly
    ...
    mem_value_t* startOfStacks = (uint8_t*)((AVR_MEMORY_SRAM / 2) + AVR_SRAM_START);
    ...
    mem_addr_t  hugeChunk = os_malloc(intHeap, os_getUseSize(intHeap));
    if (!hugeChunk) { hc_error(PSTR("f:Huge alloc fail")); }
    ...
    *startOfStacks = 0x00;
    ...
    if (*startOfStacks != 0x00) { hc_error(PSTR("Internal heap   too large")); }
...
// 3. Phase: Check if we can allocate the whole use-area after killing a RAM-hogger
    ...
    for (i = 0; i < DRIVERS; i++)
	{
        ...
        if (!hugeChunk) { hc_error(PSTR("b:Huge alloc fail")); }
		else { os_free(heap, hugeChunk); }
#endif
        ...
        hugeChunk = os_malloc(heap, os_getUseSize(heap));
        if (!hugeChunk) { hc_error(PSTR("w:Huge alloc fail")); }
		else { os_free(heap, hugeChunk); }
#endif
        ...
        hugeChunk = os_malloc(heap, os_getUseSize(heap));
        if (!hugeChunk) { hc_error(PSTR("n:Huge alloc fail")); }
		else { os_free(heap, hugeChunk); }
#endif
    }
...
// 4. Phase: Main test: Check if heap is cleaned up
    ...
    for (runs = 0; runs < totalRuns; runs++)
	{
        ...
        if ((runs + 1) % 50 == 0)
		{
			...
			{
                heap_t* heap = os_lookupHeap(i);
                mem_addr_t hugeChunk = os_malloc(heap, os_getUseSize(heap));
                // We could not allocate all the memory on that heap, so there was
                // a leak
                if (!hugeChunk) { hc_error(PSTR("Heap not clean")); }
				else { os_free(heap, hugeChunk); }
            }
