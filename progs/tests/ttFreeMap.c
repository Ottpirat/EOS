...
//          TestSuite: FreeMap
//-------------------------------------------------
// Checks whether the heap has been correctly
// initialized and the release of allocated memory
//-------------------------------------------------
#include "../progs.h"
#if defined(TESTTASK_ENABLED) && TESTTASK == TT_FREE_MAP
...
uint8_t tt_isAreaUniform(mem_addr_t start, uint16_t size, mem_value_t byte);
...
// Phase 1: Some sanity checks
    ...
    if (errorCode)
	{
		...
		{
            if (errorCode & (1 << errorIndex)) { lcd_writeProgString(PSTR("ERR|")); }
			else                               { lcd_writeProgString(PSTR("OK |")); }
        }
        HALT;
    }
...
// Phase 2: malloc
    ...
    mem_addr_t hugeChunk = os_malloc(DRIVER, os_getUseSize(DRIVER));
    if (hugeChunk == 0)
	{
...
// Phase 3: Free
    ...
    process_id_t process = os_getCurrentProc();
    ...
    uint8_t i;
    for (i = 1; i < SIZE / 2; i++) { DRIVER->driver->write(addr + i, 0xFF); }
    // Fill the first half of the block with 0xFF
    for (i = 0; i < SIZE / 2; i++) { DRIVER->driver->write(addr + (SIZE / 2) + i, 0xFF); }

    // Determine use address by calculating offset of map-entry to map start
    mem_addr_t  useaddr = ((addr - os_getMapStart(DRIVER)) * 2) + os_getUseStart(DRIVER);
...
uint8_t tt_isAreaUniform(mem_addr_t start, uint16_t size, mem_value_t byte)
{
    ...
    for (i = 0; i < size; i++)
	{
        if (DRIVER->driver->read(start + i) != byte) { return 0; }
    }
    return 1;
}
