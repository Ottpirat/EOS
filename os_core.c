...
void os_init()
{
	...
	lcd_writeProgString(PSTR("Booting EOS ..."));

	terminal_writeProgString(PSTR("\n\n##################################################\n"));
	INFO("Booting EOS ...");
	INFO("Used global vars: %d/%d bytes", (uint16_t)&__heap_start - AVR_SRAM_START, HEAP_OFFSET);
	terminal_writeProgString(PSTR("--------------------------------------------------\n"));

	// Security check if the stack crushes global variables
	assert((uint16_t)&__heap_start < AVR_SRAM_START + HEAP_OFFSET, "Heap collides with global vars");
